...
@register_cls_models("ehfr_net_multiscale")
class EHFR_Net_MultiScale(BaseEncoder):
    """
    Enhanced EHFR_Net architecture with multiscale color-texture auxiliary branch
    ...
    def __init__(self, opts, *args, **kwargs) -> None:
        ...
        aux_dim = getattr(opts, "model.classification.ehfr_net_multiscale.aux_dim", 32)
        gabor_channels = getattr(opts, "model.classification.ehfr_net_multiscale.gabor_channels", 16)
        fused_dim = getattr(opts, "model.classification.ehfr_net_multiscale.fused_dim", 160)
        ...
        # Main backbone - same as original EHFR_Net
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        # Original EHFR_Net arguments
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.attn-dropout",
            type=float,
            default=0.0,
            help="Dropout in attention layer. Defaults to 0.0",
        )
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.ffn-dropout",
            type=float,
            default=0.0,
            help="Dropout between FFN ehfr_net. Defaults to 0.0",
        )
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.dropout",
            type=float,
            default=0.0,
            help="Dropout in attention layer. Defaults to 0.0",
        )
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.width-multiplier",
            type=float,
            default=1.0,
            help="Width multiplier. Defaults to 1.0",
        )
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.attn-norm-layer",
            type=str,
        ...
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.aux-dim",
            type=int,
            default=32,
            help="Auxiliary branch output dimension. Defaults to 32",
        )
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.gabor-channels",
            type=int,
            default=16,
            help="Number of Gabor filter channels. Defaults to 16",
        )
        group.add_argument(
            "--model.classification.ehfr_net_multiscale.fused-dim",
            type=int,
            default=160,
    ...
    def _make_hblock(
        ...
        dropout = getattr(opts, "model.classification.ehfr_net_multiscale.dropout", 0.0)
        ...
        block.append(
            Block(
                ...
                dropout=dropout,
                ffn_dropout=getattr(
                    opts, "model.classification.ehfr_net_multiscale.ffn_dropout", 0.0
                ),
                attn_dropout=getattr(
                    opts, "model.classification.ehfr_net_multiscale.attn_dropout", 0.0
                ),
                attn_norm_layer=getattr(
                    opts, "model.classification.ehfr_net_multiscale.attn_norm_layer", "layer_norm_2d"
                ),
