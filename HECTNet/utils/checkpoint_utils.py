...
def load_checkpoint(
    ...
    if resume_loc is not None and os.path.isfile(resume_loc):
        ...
        if "model_state_dict" in checkpoint:
            model = load_state_dict(model, checkpoint["model_st\te_dict"])
        elif "model" in checkpoint:
            model = load_state_dict(model, checkpoint["model"])
        elif "state_dict" in checkpoint:
            model = load_state_dict(model, checkpoint["state_dict"])
        else:
            # If checkpoint only contains model weights directly
            model = load_state_dict(model, checkpoint)
            if is_master_node:
                logger.log("Checkpoint contains model weights directly (no wrapper key)")
        
        # Only load optimizer and gradient scalar if they exist in checkpoint
        if "optim_state_dict" in checkpoint:
            ...
            if is_master_node:
                logger.log("Optimizer state not found in checkpoint, using fresh optimizer")
                
        if "gradient_scalar_state_dict" in checkpoint:
            gradient_scalar.load_state_dict(checkpoint["gradient_scalar_state_dict"])
        elif "scaler" in checkpoint:
            gradient_scalar.load_state_dict(checkpoint["scaler"])
        else:
            if is_master_node:
                logger.log("Gradient scalar state not found in checkpoint, using fresh gradient scalar")
        ...
        elif model_ema is not None and "ema" in checkpoint:
            model_ema.ema_model = load_state_dict(
                model_ema.ema_model, checkpoint["ema"]
            )
