...
def plot_cam(image_path, cam_path, model=None, target_layer_list=None, size=None):
    ...
    # Create a proper args object instead of lambda
    class Args:
        def __init__(self):
            self.method = "gradcam"
            self.eigen_smooth = True
            self.aug_smooth = True
    
    args = Args()

    # Use provided model or create one (fallback)
    if model is None:
    ...
    # Remove use_cuda parameter from the CAM algorithm initialization
    with cam_algorithm(model=model,
        ...
        for file_name in glob.glob(image_path):
            ...
            # Remove the GuidedBackpropReLUModel section as it's causing issues
            # and is not essential for basic GradCAM visualization
